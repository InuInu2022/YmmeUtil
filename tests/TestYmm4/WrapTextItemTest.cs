...
public class WrapTextItemTest
{
	...
	public void TextPropsTest()
	{
		...
		textItem.FontSize.AddToEachValues(
			32 - textItem.FontSize.DefaultValue
		);
		...
		Assert.Equal(32.0, textItem.FontSize.DefaultValue);
	...
	public void TextSetPropsTest()
	{
		...
		Assert.Equal(
			"変更されたテキスト",
			textItem.RawItem.Text
		);
		Assert.Equal("メイリオ", textItem.RawItem.Font);
		Assert.Equal(
			Colors.Red,
			textItem.RawItem.FontColor
		);
